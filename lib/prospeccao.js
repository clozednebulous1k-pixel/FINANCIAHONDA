...
function porteDe(tags, nome, site) {
  ...
  if (/dentist|pharmacy|restaurant|veterinary|cafe|fast_food/.test(amenity) || /car_repair|tyres|clothes|shoes/.test(shop)) {
...
function montarEmpresa({ nome, fone, cidade, bairro, endereco, categoria, site, id, lat, lng, porte }) {
  ...
  return {
    ...
    porte,
    motivo: semSite ? "Celular no mapa · sem site" : "Celular no mapa",
    lat,
    lng,
  };
}
...
export async function vasculharEmpresas({
  ...
  const portePreferido = Math.max(etapa.nivel, 2);
  ...
  const fila = (preferidas.length >= Math.min(max, 8) ? preferidas : candidatos).slice(0, max);
  return {
    cidade: lugar,
    segmento: seg,
    total: fila.length,
