...
function telefonesDe(fonte) {
  ...
  const movel = achados.map((d) => celularWhatsapp(d)).find(Boolean);
  return movel || "";
}
...
function montarEmpresa({ nome, fone, cidade, bairro, endereco, categoria, site, id, lat, lng, porte }) {
  ...
  return {
    ...
    whatsapp: celularWhatsapp(local) || validarWhatsapp(local) || local,
    whatsappOk: Boolean(celularWhatsapp(local)),
  ...
  for (let i = 0; i < rota.length && i < maxConsultas; i += 1) {
    ...
    try {
      const extra = await overpass(
        queryOverpass(ponto.lat, ponto.lng, segmento, ponto.raio || 8000, 4),
      );
      mapaOk = true;
      absorver(extra);
    } catch (error) {
