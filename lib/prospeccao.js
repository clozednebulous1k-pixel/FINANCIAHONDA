...
async function nominatimBusca(termo, cidade) {
  ...
  const res = await fetch(
    `https://nominatim.openstreetmap.org/search?format=jsonv2&limit=20&addressdetails=1&extratags=1&countrycodes=br&q=${q}`,
...
export async function vasculharEmpresas({ cidade = "São Paulo", segmento = "todos" } = {}) {
  ...
  function incluir(item) {
    ...
    const chave = `${item.nome.toLowerCase()}|${item.whatsapp}`;
  ...
  for (const termo of termos.slice(0, 3)) {
    ...
    for (const hit of hits) incluir(empresaDoNominatim(hit, lugar));
    if (empresas.length >= 40) break;
    await new Promise((r) => setTimeout(r, 900));
  }

  if (empresas.length < 8) {
    try {
      const geo = await geocodificarCidade(lugar);
      const osm = await overpassRapido(geo.lat, geo.lng, segmento);
      for (const el of osm) incluir(empresaDoOsm(el, lugar));
    } catch {
      // Nominatim já devolveu o que tinha
    }
  }

  empresas.sort((a, b) => Number(b.whatsappOk) - Number(a.whatsappOk) || a.nome.localeCompare(b.nome));
  const lista = empresas.slice(0, 40);
  return {
    cidade: lugar,
    total: lista.length,
    comWhatsapp: lista.filter((e) => e.whatsappOk).length,
    empresas: lista,
  };
}
