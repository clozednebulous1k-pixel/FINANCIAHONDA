...
export function leadJaFoiChamado(lead) {
  ...
  if (lead.disparoReservadoEm) return true;
...
export async function jaEnviouMensagem(leadId) {
  ...
  const nossas = await getDocs(
    query(collection(db, "leads", id, "mensagens"), where("fromMe", "==", true), limit(1)),
  );
  return !nossas.empty;
}
...
export async function salvarMensagem(leadId, { texto, fromMe, messageId }) {
  ...
  if (mid) {
    const iguais = await getDocs(
      query(collection(db, "leads", id, "mensagens"), where("messageId", "==", mid), limit(1)),
    );
    if (!iguais.empty) return;
  }
...
export async function apagarConversa(leadId) {
  ...
  const snap = await getDocs(
    query(collection(db, "leads", id, "mensagens"), orderBy("createdAt", "asc"), limit(400)),
  );
 ...
 */
export async function podarHistoricoAntesDaChamada(leadId, lead) {
  if (!db) throw new Error("Firebase não configurado");
  const id = validarId(leadId);
  if (!id) return 0;
  if (lead?.conversaDesde) return 0;
