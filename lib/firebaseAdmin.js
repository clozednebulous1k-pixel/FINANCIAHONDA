...
export async function acharLeadPorWhatsapp(numero) {
  ...
  const variantes = [...new Set([alvo, `55${alvo}`, numero ? String(numero).replace(/\D/g, "") : ""])].filter(
    (v) => v.length >= 10,
  );
  for (const valor of variantes) {
    const snap = await db.collection("leads").where("whatsapp", "==", valor).limit(1).get();
    if (!snap.empty) {
...
export async function leadJaRecebeuMensagemNossa(leadId) {
  const db = getAdminDb();
