...
export async function importarLeadsCnh() {
  ...
  const confirmado = await getDocsFromServer(collection(db, "leads"));
  const salvos = new Set(confirmado.docs.map((item) => String(item.data().whatsapp || "").replace(/\D/g, "")));
  const aindaFalta = LEADS_CNH.some((lead) => !salvos.has(lead.whatsapp));
  if (aindaFalta) {
