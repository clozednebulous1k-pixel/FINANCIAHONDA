  ...
  return onSnapshot(
    ...
    (snap) => {
      const lista = snap.docs.map((item) => ({
        id: item.id,
        ...item.data(),
        _pendente: item.metadata.hasPendingWrites,
      }));
      lista.sort((a, b) => {
        if (Boolean(a._pendente) !== Boolean(b._pendente)) {
          return a._pendente ? -1 : 1;
        }
...
export async function importarLeadsCnh() {
  ...
  const porWhatsapp = new Map(
    snap.docs.map((item) => [String(item.data().whatsapp || "").replace(/\D/g, ""), { id: item.id, ...item.data() }]),
  );
  const nota = "Tráfego pago · tem CNH · ainda sem resposta de moto/interesse";

  for (const lead of LEADS_CNH) {
    const atual = porWhatsapp.get(lead.whatsapp);
    if (!atual) {
      await criarLead({
        nome: lead.nome,
    ...
    if (atual.tipo !== "CONSTATANDO" || atual.observacao !== nota) {
      await updateDoc(doc(db, "leads", atual.id), {
        tipo: "CONSTATANDO",
        observacao: nota,
      });
    }
