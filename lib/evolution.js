...
/** Em Vercel, localhost da Evolution não responde — não tenta checar Zap lá. */
export function evolutionRemota() {
  const url = baseUrl();
...
function msgAbort(error, fallback = "Demorou demais, tenta de novo") {
  const nome = error?.name || "";
  const msg = String(error?.message || "");
  if (
    nome === "AbortError" ||
    /aborted|abort|timed?\s*out|demorou/i.test(msg)
  ) {
    return fallback;
  }
  return msg || fallback;
...
async function evoFetch(path, { method = "GET", body, timeoutMs = 25000 } = {}) {
  ...
  try {
    ...
    const err = new Error(msgAbort(error, "WhatsApp demorou pra responder. Tenta de novo."));
    err.status = 504;
    throw err;
  } finally {
    clearTimeout(timer);
  ...
  if (!res.ok) {
    const bruto = data?.response?.message || data?.message || data?.error || data?.raw;
    const extraido = Array.isArray(bruto) ? bruto.filter(Boolean).join(" ") : bruto;
    let msg = typeof extraido === "string" && extraido.trim() ? extraido.trim() : `HTTP ${res.status}`;
    if (res.status === 400 || /^bad request$/i.test(msg)) {
      msg = "Esse número não está no WhatsApp (telefone fixo ou inválido).";
    }
    const err = new Error(msgAbort({ message: msg }, msg));
...
/** Confere se o número tem WhatsApp. ok=false = API falhou (não tratar como sem Zap). */
export async function numeroTemWhatsapp(numero, conta = "honda") {
  const digits = String(numero || "").replace(/\D/g, "");
  if (digits.length < 12) return { ok: true, exists: false, numero: digits };
  const mapa = await checarNumerosWhatsapp([digits], conta);
  const item = mapa.get(digits);
  if (!item) return { ok: false, exists: true, numero: digits };
  return { ok: true, exists: item.exists, numero: item.numero, raw: item.raw };
}

/** Checa vários números de uma vez. Retorna Map<digits, {exists, numero}>. */
export async function checarNumerosWhatsapp(numeros, conta = "honda") {
  const lista = [
    ...new Set(
  ...
  const data = await evoFetch(`/chat/whatsappNumbers/${nomeInstancia(conta)}`, {
    method: "POST",
    body: { numbers: lista },
    timeoutMs: 15000,
  });
  const rows = Array.isArray(data)
    ...
    : Array.isArray(data?.numbers)
      ...
      : Array.isArray(data?.data)
        ...
        : [];
  if (!rows.length) {
    throw new Error("Checagem WhatsApp sem resposta");
  }

  function acharRow(digits) {
    const limpo = (v) => String(v || "").replace(/\D/g, "");
    const direto = rows.find((x) => limpo(x?.number) === digits);
    if (direto) return direto;
    const porJid = rows.find((x) => limpo(x?.jid).includes(digits.slice(-8)));
    if (porJid) return porJid;
    return rows.find((x) => {
      const n = limpo(x?.number) || limpo(x?.jid);
      return n && (n.endsWith(digits.slice(-8)) || digits.endsWith(n.slice(-8)));
    });
  }

  for (const digits of lista) {
    const item = acharRow(digits);
    if (!item) {
      out.set(digits, { exists: false, numero: digits, raw: null });
      continue;
    }
    const flag = item?.exists ?? item?.isWhatsApp ?? item?.whatsapp;
    const doJid = String(item?.jid || "")
      .split("@")[0]
      .replace(/\D/g, "");
    const exists =
      flag === true ||
      flag === "true" ||
      flag === 1 ||
      (flag == null && Boolean(doJid)) ||
      (flag !== false && flag !== "false" && Boolean(doJid));
    out.set(digits, {
      exists: Boolean(exists),
      numero: exists && doJid.length >= 12 ? doJid : digits,
      raw: item,
    });
  }
  return out;
...
/** Mantém só empresas cujo número existe no WhatsApp (Evolution). */
export async function filtrarEmpresasComWhatsapp(empresas, conta = "agencia") {
  const lista = Array.isArray(empresas) ? empresas : [];
  if (!lista.length) return [];
  const e164s = lista.map((e) => {
    const local = String(e?.whatsapp || "").replace(/\D/g, "");
    return local.startsWith("55") ? local : `55${local}`;
  });
  let mapa;
  try {
    mapa = await checarNumerosWhatsapp(e164s, conta);
  } catch {
    // Check fora: libera celular do mapa; o envio confirma (400 = sem Zap)
    return lista.filter((e) => e.whatsappOk).slice(0, 10);
  }
  const ok = [];
  for (let i = 0; i < lista.length; i += 1) {
    ...
    const local = String(info.numero || key).replace(/\D/g, "").replace(/^55/, "");
    ok.push({
      ...emp,
      whatsapp: local.slice(0, 11),
      whatsappOk: true,
      motivo: emp.motivo ? `${emp.motivo} · Zap ok` : "WhatsApp confirmado",
    });
  }
  // Se a API marcou todos como false (formato estranho), não zera o lote
  if (!ok.length && lista.some((e) => e.whatsappOk)) {
    return lista.filter((e) => e.whatsappOk).slice(0, 10);
  }
  return ok;
}
...
export async function enviarTexto(numero, texto, conta = "honda") {
  ...
  const typing = Math.min(
    4200,
    (conta === "agencia" ? 1400 : 800) + Math.floor(chars * 12) + Math.floor(Math.random() * 1200),
  );
