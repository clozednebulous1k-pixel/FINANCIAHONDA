...
export async function checarNumerosWhatsapp(numeros, conta = "honda") {
  ...
  for (const digits of lista) {
    const item =
      rows.find((x) => String(x?.number || "").replace(/\D/g, "") === digits) ||
      rows.find((x) => String(x?.jid || "").replace(/\D/g, "").includes(digits.slice(-8))) ||
      null;
    if (!item) {
...
export async function filtrarEmpresasComWhatsapp(empresas, conta = "agencia") {
  ...
  const e164s = lista.map((e) => {
    ...
    const com55 = local.startsWith("55") ? local : `55${local}`;
    return com55;
  });
  let mapa;
  try {
    ...
    // API de check fora: não libera lote sem confirmar Zap
    return [];
  }
  const ok = [];
