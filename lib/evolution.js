...
export async function enviarImagemDestino(destino, urlImagem, caption, conta = "honda") {
  ...
  const midia = await prepararMidiaImagem(url);
  const payload = {
    number: destinoWhatsapp(destino),
    mediatype: "image",
  ...
  try {
    ...
    if (midia.media !== url && /^https:\/\//i.test(url)) {
      return evoFetch(`/message/sendMedia/${nomeInstancia(conta)}`, {
        method: "POST",
        body: { ...payload, media: url.slice(0, 2000) },
      });
    }
    throw error;
  }
...
async function prepararMidiaImagem(urlImagem) {
  const url = String(urlImagem || "").trim();
  const fileName = /\.webp$/i.test(url) ? "achadinho.webp" : "achadinho.jpg";
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (buf.length >= 2000 && buf.length <= 4_500_000) {
        const tipo = String(res.headers.get("content-type") || mimetype).split(";")[0];
        const nome = tipo.includes("webp") ? "achadinho.webp" : "achadinho.jpg";
        return {
          media: buf.toString("base64"),
          mimetype: tipo.startsWith("image/") ? tipo : mimetype,
          fileName: nome,
        };
      }
    }
  } catch {
    // tenta mandar a URL direto
  }
  return { media: url.slice(0, 2000), mimetype, fileName };
}
