...
export async function enviarTextoDestino(destino, texto) {
  return evoFetch(`/message/sendText/${DEFAULTS.instance}`, {
    method: "POST",
    body: {
...
export async function enviarImagemDestino(destino, urlImagem, caption) {
  return evoFetch(`/message/sendMedia/${DEFAULTS.instance}`, {
    method: "POST",
    body: {
      number: destinoWhatsapp(destino),
      mediatype: "image",
...
export async function listarGruposWhatsapp() {
  return evoFetch(`/group/fetchAllGroups/${DEFAULTS.instance}?getParticipants=false`);
}
