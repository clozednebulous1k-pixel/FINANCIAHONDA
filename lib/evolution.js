  ...
  const lista = Array.isArray(data)
    ...
    : Array.isArray(data?.numbers)
      ...
      : Array.isArray(data?.data)
        ...
        : [];
  if (!lista.length) {
    // Resposta vazia/ambígua — não bloqueia o envio
    return { ok: false, exists: true, numero: digits };
  }
  const item =
    lista.find((x) => String(x?.number || "").replace(/\D/g, "") === digits) ||
    lista.find((x) => String(x?.jid || "").includes(digits.slice(-8))) ||
    lista[0] ||
    null;
