...
/** Confere se o número tem WhatsApp antes de disparar (evita Bad Request). */
export async function numeroTemWhatsapp(numero, conta = "honda") {
  const digits = String(numero || "").replace(/\D/g, "");
  if (digits.length < 12) return { exists: false, numero: digits };
  ...
  return {
    exists,
    numero: exists && doJid ? doJid : digits,
    raw: item,
  };
}
