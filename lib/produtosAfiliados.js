...
export { montarTextoOferta } from "./textoAchadinho";
...
function precoDoCard(chunk) {
  const fracs = [...String(chunk).matchAll(/andes-money-amount__fraction[^>]*>([^<]+)/g)]
    .map((m) => m[1].replace(/\D/g, ""))
    .filter(Boolean);
  const cents = [...String(chunk).matchAll(/andes-money-amount__cents[^>]*>([^<]+)/g)]
    .map((m) => m[1].replace(/\D/g, ""));
  const valores = fracs
    .map((f, i) => {
      const c = String(cents[i] || "00").padEnd(2, "0").slice(0, 2);
      return Number(`${f}.${c}`);
    })
    .filter((n) => Number.isFinite(n) && n > 0);
  if (!valores.length) return { preco: 0, de: 0 };
  const preco = Math.min(...valores);
  const de = Math.max(...valores);
  return { preco, de: de > preco * 1.02 ? de : 0 };
}

function parsePolyCards(html, tag, word = "") {
  const pedacos = String(html || "").split(/poly-card/);
  const produtos = [];
  const vistos = new Set();
  for (const pedaco of pedacos) {
    const titulo = decodeHtml(pedaco.match(/poly-component__title[^>]*>([^<]{8,180})/)?.[1] || "");
    const hrefBruto = pedaco.match(/href="(https:\/\/(?:www|produto)\.mercadolivre\.com\.br\/[^"]+)"/)?.[1] || "";
    const href = decodeHtml(hrefBruto).split("#")[0];
    if (!titulo || !/mercadolivre\.com\.br\//i.test(href) || /\/c\/|\/gz\/|\/cart/i.test(href)) continue;
    const id =
      href.match(/MLB-?\d+/)?.[0]?.replace("-", "") ||
      href.match(/\/p\/(MLB\d+)/i)?.[1] ||
      href;
    if (vistos.has(id)) continue;
    vistos.add(id);
    const { preco, de } = precoDoCard(pedaco);
    const offTxt = Number(pedaco.match(/(\d+)\s*%\s*OFF/i)?.[1] || 0);
    const imagem = pedaco.match(/https:\/\/http2\.mlstatic\.com\/D_[^"'\\\s]+\.(?:jpg|jpeg|webp|png)/i)?.[0] || "";
    const item = produtoMl(
      {
        id,
        titulo,
        preco,
        de: de || (offTxt && preco ? preco / (1 - offTxt / 100) : 0),
        imagem,
    ...
    if (offTxt > item.desconto) item.desconto = offTxt;
    produtos.push(item);
  }
  return produtos;
}
...
function produtoMl({ id, titulo, preco, de, imagem, permalink }, tag, word = "") {
  ...
  return {
    ...
    desconto: off,
    imagem: normalizarUrlImagemMl(imagem),
...
function normalizarUrlImagemMl(url) {
  let u = String(url || "")
    .replace(/&amp;/g, "&")
    .replace("http://", "https://")
    .trim();
  if (!u) return "";
  u = u.split(" ")[0].split("#")[0];
  u = u.replace(/-[ISY]\.(jpe?g|webp|png)/i, "-O.$1");
  u = u.replace(/-W\.webp/i, "-O.jpg");
...
async function buscarMercadoLivreListado(termo, tag, word = "") {
  const html = await htmlMercadoLivre(`https://lista.mercadolivre.com.br/${slugBusca(termo)}`);
  const cards = parsePolyCards(html, tag, word);
...
async function itemMercadoLivre(id, tag, word = "") {
  ...
  try {
    const html = await htmlMercadoLivre(`https://www.mercadolivre.com.br/p/${mlb}`);
    const titulo =
      decodeHtml(html.match(/property="og:title" content="([^"]+)"/i)?.[1] || "") ||
      decodeHtml(html.match(/<h1[^>]*>([^<]{8,180})<\/h1>/i)?.[1] || "");
    const imagem = html.match(/property="og:image" content="([^"]+)"/i)?.[1] || "";
    const preco = Number(
    ...
    return produtoMl({ id: mlb, titulo, preco, de: 0, imagem, permalink }, tag, word);
  } catch {
    return null;
  }
}

function idsMlbDoHtml(html) {
  const ids = new Set();
  for (const m of String(html || "").matchAll(/MLB-?(\d{8,13})/gi)) {
    ids.add(`MLB${m[1]}`);
  }
  return [...ids];
}

async function htmlMercadoLivre(url) {
  let ultimoErro = "Falha ao abrir o Mercado Livre";
  for (const ua of UAS_ML) {
    try {
      ...
      if (/account-verification/i.test(html) || html.length < 15000) {
        ultimoErro = "página de verificação";
        continue;
      }
      return html;
    } catch (error) {
      ultimoErro = error.message || "rede";
    }
  }
  throw new Error(ultimoErro);
}

async function detalhesItensMlb(ids, tag, word = "") {
  const unicos = [...new Set(ids)].slice(0, 80);
  const produtos = [];
  for (let i = 0; i < unicos.length; i += 20) {
    const lote = unicos.slice(i, i + 20);
    try {
      const res = await fetch(`https://api.mercadolibre.com/items?ids=${lote.join(",")}`, {
        cache: "no-store",
        headers: HEADERS_ML,
      });
      if (!res.ok) continue;
          ...
          produtoMl(
            {
              ...
              preco: item.price,
              de: item.original_price,
              imagem: item.secure_thumbnail || item.thumbnail || item.pictures?.[0]?.secure_url,
              permalink: item.permalink,
            },
            tag,
            word,
...
function ranquearPrecoBom(lista) {
  return [...lista].sort((a, b) => {
    const da = Number(a.desconto || 0);
    const db = Number(b.desconto || 0);
...
async function roboPrecosBonsMeli(tag, word = "") {
  const paginas = [
    "https://www.mercadolivre.com.br/ofertas",
    "https://lista.mercadolivre.com.br/ofertas-do-dia",
    "https://lista.mercadolivre.com.br/promocao",
    "https://lista.mercadolivre.com.br/creatina",
    "https://lista.mercadolivre.com.br/fone-bluetooth",
  ];
  const htmls = await Promise.allSettled(paginas.map((url) => htmlMercadoLivre(url)));
  const ids = new Set();
  const fallback = [];

  for (const r of htmls) {
    if (r.status !== "fulfilled") continue;
    fallback.push(...parsePolyCards(r.value, tag, word));
    for (const id of idsMlbDoHtml(r.value)) ids.add(id);
    try {
      fallback.push(...buscarMercadoLivreListadoFromHtml(r.value, tag, word));
    } catch {
      // segue
    }
  }

  const detalhados = await detalhesItensMlb([...ids], tag, word);
  const base = detalhados.length ? detalhados : fallback;
  const limpos = filtrarOfertas(base, false);
  const comDesconto = limpos.filter((item) => item.desconto >= 15);
  const baratos = limpos.filter((item) => item.preco > 0 && item.preco <= 80).slice(0, 12);
  const unicos = new Map();
  for (const item of [...comDesconto, ...baratos, ...limpos]) {
    if (!unicos.has(item.id)) unicos.set(item.id, item);
  }
  return ranquearPrecoBom([...unicos.values()]).slice(0, 36);
}

function buscarMercadoLivreListadoFromHtml(html, tag, word = "") {
  const produtos = [];
  const ldBlocks = [...html.matchAll(/<script type="application\/ld\+json">([\s\S]*?)<\/script>/gi)];
  for (const bloco of ldBlocks) {
    try {
...
async function buscarMercadoLivre(termo, soPromo, tag, word = "") {
  ...
  const ids = listado
    .map((p) => String(p.id || p.link || "").match(/MLB-?\d+/i)?.[0]?.replace("-", "").toUpperCase())
    .filter(Boolean);
  if (ids.length) {
    const detalhados = await detalhesItensMlb(ids, tag, word);
    if (detalhados.length) return filtrarOfertas(detalhados, soPromo);
  }
...
export async function buscarProdutosAfiliados({
  ...
  const querShopee = origem !== "mercadolivre" && !robo;
  const avisos = [];
  let ml = [];
  let shopee = [];
  const tool = meliTag || process.env.MELI_AFFILIATE_ID || "62300245";
  const word = meliWord || process.env.MELI_AFFILIATE_WORD || "bbhgadcfe38621";

  if (querMl) {
    try {
      ml = robo
        ? await roboPrecosBonsMeli(tool, word)
        : await buscarMercadoLivre(q, soPromo, tool, word);
      if (robo && !ml.length) {
        avisos.push("Robô: poucas ofertas com desconto agora. Tente de novo ou cole o link do produto.");
      }
    } catch (error) {
