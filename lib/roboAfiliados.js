...
import { evolutionConfigurado, enviarOfertaGrupo } from "./evolution";
...
async function postarNoGrupo(grupo, produto, gancho) {
  ...
  for (const destino of destinos.slice(0, 1)) {
    await enviarOfertaGrupo(destino, { texto, imagem }, "afiliados");
  }
}
...
export async function rodarTickRoboAfiliados() {
  ...
  if (agora.hora < horaIni || agora.hora >= horaFim) {
    const msg = `Pausado: fora do horário (${horaIni}h–${horaFim}h)`;
    if (cfg.autoStatus !== msg) await ref.set({ autoStatus: msg }, { merge: true });
  ...
  if (msgsDia >= maxMsgs || produtosDia >= ritmo.maxProdutos) {
    const msg = `Limite do dia: ${produtosDia} achadinhos, ${msgsDia} envios.`;
    if (cfg.autoStatus !== msg || cfg.autoDia !== agora.dia) {
      await ref.set({ autoStatus: msg, autoDia: agora.dia }, { merge: true });
    }
  ...
  if (!fila.length || !produto) {
    ...
    const cacheGrupos = Array.isArray(cfg.autoGrupos) ? cfg.autoGrupos : [];
    const cacheNovo = Date.now() - Number(cfg.autoGruposEm || 0) < 6 * 60 * 60 * 1000;
    let grupos = cacheNovo ? cacheGrupos.filter((g) => g?.ativo !== false).slice(0, ritmo.maxGrupos) : [];
    if (!grupos.length) {
      const gruposSnap = await db.collection("afiliados_grupos").limit(40).get();
      ...
      await ref.set(
        {
          autoGrupos: grupos.map((g) => ({
            id: g.id,
            nome: g.nome,
            tipo: g.tipo,
            jid: g.jid || "",
            pessoas: Array.isArray(g.pessoas) ? g.pessoas.slice(0, 20) : [],
            ativo: true,
          })),
          autoGruposEm: Date.now(),
        },
        { merge: true },
      );
    }
    if (!grupos.length) {
      if (cfg.autoStatus !== "Ligado, mas não há grupo ativo.") {
    ...
    produto =
      ...
      null;
    if (!produto) {
      const msg = "Não achei oferta nova agora. Tento de novo no próximo ciclo.";
      if (cfg.autoStatus !== msg) await ref.set({ autoStatus: msg }, { merge: true });
