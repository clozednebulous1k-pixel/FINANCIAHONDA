...
export async function jaExisteLeadAgencia(whatsapp) {
  if (!db) return false;
  const fone = validarWhatsapp(whatsapp);
  if (!fone) return false;
  const snap = await getDocs(query(collection(db, COL), where("whatsapp", "==", fone), limit(1)));
  return !snap.empty;
}

export async function salvarLeadAgencia(dados) {
  ...
  if (await jaExisteLeadAgencia(whatsapp)) return { duplicado: true, whatsapp };
...
export async function atualizarLeadAgencia(id, dados) {
  ...
  if (dados.ultimaMensagem != null) patch.ultimaMensagem = textoSeguro(dados.ultimaMensagem, 180);
