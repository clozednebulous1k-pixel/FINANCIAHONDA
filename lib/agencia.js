...
import { celularWhatsapp, chaveEmpresa, chaveWhatsapp, textoSeguro, validarId, validarStatus, validarWhatsapp } from "./security";
...
export async function salvarLeadAgencia(dados, conhecidos = []) {
  ...
  const nome = textoSeguro(dados.nome, 120);
  const whatsapp = celularWhatsapp(dados.whatsapp);
  if (!nome || !whatsapp) {
    await registrarVistoAgencia(dados, "visto");
    return { duplicado: true, whatsapp: chaveWhatsapp(dados.whatsapp), pulado: true };
  }
